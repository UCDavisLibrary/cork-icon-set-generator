...
class FontAwesomeExtractor {
  async run(name, dir, iconSetType, useViewBox){
    ...
    // Extract whole set
    if ( ICONS.sets ) {
      for (let s of ICONS.sets ) {
        let setPath = path.resolve(FONTAWESOME_DIR, `svgs/${s}`);
        if ( !fs.pathExistsSync(setPath) ){
          console.warn(`${s} iconset does not exist. Skipping...`);
          continue;
        }
        for (let file of fs.readdirSync(setPath)){
          let name = `fa-${file.split(".")[0]}`;
          let svg = await crawler.getIconSvg(name, path.resolve(setPath, file), useViewBox);
          iconSet.add(name, svg);
        };
      }
    }

    // Extract individual svgs
    if ( ICONS.icons ){
